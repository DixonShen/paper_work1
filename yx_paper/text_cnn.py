...
class TextCNN_YX(object):
    def __init__(self, whether_word2vec, whether_tf, vocab_size, embedding_dim, sequence_length, tf_dict_size, tf_emb_size, rw_dict_size, rw_emb_size, num_classes, filter_sizes,
        ...
        if whether_tf:
            self.tf = tf.placeholder(tf.int32, [None, sequence_length], name='tf')
        self.rw = tf.placeholder(tf.int32, [None, sequence_length], name="rw")
        self.input_x = tf.placeholder(tf.int32, [None, sequence_length], name="input_x") #词的
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            if whether_tf:
                ...
                t_tf_emb = tf.nn.embedding_lookup(ini_tf_emb, self.tf)
                self.X = tf.concat([self.X, t_tf_emb], 2)   # 更新X
                sum_dim += tf_emb_size    # 更新sum dimension

            #rw
            ini_rw_emb = tf.Variable(tf.random_uniform([rw_dict_size, rw_emb_size], -1.0, +1.0), 'rw_vec')
            t_rw_emb = tf.nn.embedding_lookup(ini_rw_emb, self.rw)
            self.X = tf.concat([self.X, t_rw_emb], 2)   # 更新X
            sum_dim += rw_emb_size    # 更新sum dimension
